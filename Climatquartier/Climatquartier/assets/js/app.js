        ...
        this.init();
            ...
            updateChart() {
                try {
                    ...
                    const { kpi, label, unit } = this.getIndicatorMeta(indicator);
                    if (!kpi) return;
            ...
            getSelectedIndicatorMeta() {
                const active = document.querySelector('.indicator-card.active');
                const indicator = active?.dataset?.indicator || 'temp';
                return this.getIndicatorMeta(indicator);
            }
			...
			async exportChartToPDF() {
                ...
                const indicatorCards = Array.from(document.querySelectorAll('.indicator-card[data-indicator], #inonde-indicator'))
                    .filter(card => window.getComputedStyle(card).display !== 'none');

                const valueRows = indicatorCards
                    .map(card => {
                        const indicator = card.dataset.indicator || 'inonde';
                        const name = card.querySelector('.indicator-name')?.textContent?.trim();
                        const desc = card.querySelector('.indicator-desc')?.textContent?.trim() || '';
                        const value = card.querySelector('.value-main')?.textContent?.trim();
                        const change = card.querySelector('.value-change')?.textContent?.trim() || '';
                ...
                const chartIndicators = Array.from(document.querySelectorAll('.indicator-card[data-indicator]'))
                    .map(card => {
                        const indicator = card.dataset.indicator;
                        const meta = this.getIndicatorMeta(indicator);
                        const displayName = card.querySelector('.indicator-name')?.textContent?.trim() || meta.label;
                        return {
                            indicator,
                            meta,
                            displayName,
                            currentValue: card.querySelector('.value-main')?.textContent?.trim() || ''
                        };
                    })
                    .filter(item => item?.meta?.kpi);

                // Page 1: récapitulatif global des indicateurs
                pdf.setFontSize(16);
                pdf.setTextColor(26, 97, 83);
                pdf.text(`Rapport Simulation : ${city.name}`, 15, 20);

                pdf.setFontSize(12);
                pdf.setTextColor(100);
                pdf.text(`Scénario : ${this.currentScenario.toUpperCase()} | Horizon : ${this.getHorizonLabel()}`, 15, 30);
                pdf.text(`Synthèse des indicateurs`, 15, 40);

                const pageWidth = pdf.internal.pageSize.getWidth();
                const margin = 15;
                const gap = 8;
                const columns = 2;
                const cardW = (pageWidth - (margin * 2) - gap) / columns;
                const cardH = 30;
                const startY = 48;
                ...
                valueRows.forEach((row, index) => {
                    ...
                    const y = startY + (rowIdx * (cardH + 5));
                    const visual = this.getIndicatorVisual(row.indicator);
                    ...
                    pdf.setFillColor(...visual.accent);
                    pdf.roundedRect(x, y, 3, cardH, 1, 1, 'F');

                    pdf.setFillColor(...visual.iconBg);
                    pdf.roundedRect(x + 6, y + 6, 11, 11, 2, 2, 'F');
                    pdf.setTextColor(...visual.iconText);
                    pdf.setFontSize(9);
                    pdf.setFont('helvetica', 'bold');
                    pdf.text(visual.symbol, x + 11.5, y + 13, { align: 'center' });

                    pdf.setTextColor(30, 41, 59);
                    pdf.setFontSize(10);
                    pdf.text(row.name, x + 21, y + 10);
                    pdf.setFont('helvetica', 'normal');
                    pdf.setTextColor(100, 116, 139);
                    pdf.setFontSize(8);
                    pdf.text(row.desc || ' ', x + 21, y + 16);

                    pdf.setFont('helvetica', 'bold');
                    pdf.setTextColor(15, 23, 42);
                    pdf.setFontSize(12);
                    pdf.text(row.value, x + cardW - 5, y + 12, { align: 'right' });
                    ...
                    pdf.text(row.change || ' ', x + cardW - 5, y + 18, { align: 'right' });
                });

                pdf.setFontSize(10);
                pdf.setTextColor(150);
                pdf.text(`Généré par ClimatQuartier le ${new Date().toLocaleDateString()}`, 15, 200);
                ...
                // Pages suivantes: un graphique pour chaque indicateur
                for (const item of chartIndicators) {
                    const [ssp2, ssp5] = await Promise.all([
                        this.fetchChartSeries(city.name, 'SSP2', item.meta.kpi),
                        this.fetchChartSeries(city.name, 'SSP5', item.meta.kpi)
                    ]);

                    const chart = new Chart(renderCtx, {
                        type: 'line',
                        data: {
                            labels: ssp2.labels,
                            datasets: [
                        ...
                        options: {
                            ...
                            plugins: {
                                title: {
                                    display: true,
                                    text: `Projection ${item.meta.label} - ${city.name}`,
                                    color: '#1e293b',
                            ...
                            scales: {
                                y: {
                                    ...
                                    title: {
                                        display: true,
                                        text: item.meta.unit ? `${item.meta.label} (${item.meta.unit})` : item.meta.label,
                                        color: '#1e293b'
                                    }
                                },
                                x: {
                    ...
                    chart.update('none');
                    const canvasImage = renderCanvas.toDataURL('image/png', 1.0);
                    chart.destroy();

                    pdf.addPage();
                    pdf.setFontSize(14);
                    pdf.setTextColor(26, 97, 83);
                    pdf.text(`Indicateur : ${item.displayName}`, 15, 20);

                    pdf.setFontSize(11);
                    pdf.setTextColor(70);
                    if (item.currentValue) {
                        pdf.text(`Valeur actuelle affichée : ${item.currentValue}`, 15, 28);
                    }

                    pdf.addImage(canvasImage, 'PNG', 15, 35, 260, 140);
                    pdf.setFontSize(10);
                    pdf.setTextColor(150);
                    pdf.text(`Généré par ClimatQuartier le ${new Date().toLocaleDateString()}`, 15, 185);
                }

                pdf.save(`ClimatQuartier_${city.name}_Rapport_Complet.pdf`);
            }
